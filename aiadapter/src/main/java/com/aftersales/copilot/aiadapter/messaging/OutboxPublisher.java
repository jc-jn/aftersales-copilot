...
@Component public class OutboxPublisher {
 ...
 @Scheduled(fixedDelayString="${app.ai.outbox-interval-ms:1000}") public void publish(){
  List<Map<String,Object>> rows=jdbc.queryForList("SELECT event_id,routing_key,payload FROM outbox_event WHERE status IN ('NEW','FAILED') AND (next_retry_at IS NULL OR next_retry_at<=UTC_TIMESTAMP(3)) ORDER BY created_at LIMIT 50");
