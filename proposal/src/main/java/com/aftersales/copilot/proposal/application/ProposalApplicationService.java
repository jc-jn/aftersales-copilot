...
public class ProposalApplicationService {
    ...
    @Transactional public Object confirm(AuthenticatedUser user, long proposalId, ProposalRequests.Confirm request, String idempotencyKey) {
        ...
        String result; if (proposal.type().equals("REFUND_ONLY")) result = executeRefund(user, proposal, ticket, request.ticketVersion(), now); else result = createReturnFlow(proposal, ticket, request.ticketVersion(), now);
    ...
    private String createReturnFlow(ProposalMapper.ProposalView proposal, ProposalMapper.TicketContext ticket, int ticketVersion, LocalDateTime now) {
        if (mapper.transitionTicket(ticket.id(), ticket.status(), "WAITING_RETURN", ticketVersion, now) != 1) throw ProposalException.ticketVersion();
        long returnId = ids.nextId(); String returnNo = "RT" + now.toLocalDate().toString().replace("-", "") + String.format("%06d", Math.floorMod(returnId, 1_000_000)); mapper.insertReturn(returnId, returnNo, ticket.id(), proposal.id(), ticket.orderItemId(), now);
        if (proposal.type().equals("EXCHANGE")) { long exchangeId = ids.nextId(); mapper.insertExchange(exchangeId, "EX" + String.format("%010d", Math.floorMod(exchangeId, 10_000_000_000L)), proposal.id(), now); }
        if (proposal.type().equals("REPAIR")) { long repairId = ids.nextId(); mapper.insertRepair(repairId, "RP" + String.format("%010d", Math.floorMod(repairId, 10_000_000_000L)), proposal.id(), now); }
        return "{\"status\":\"WAITING_RETURN\",\"returnNo\":\"" + returnNo + "\"}";
    }

    @Transactional public Object shipReturn(AuthenticatedUser user, long returnId, ProposalRequests.Shipment request) { if (user.role() != UserRole.CUSTOMER) throw ProposalException.forbidden(); var item = mapper.findReturn(returnId).orElseThrow(ProposalException::notFound); var ticket = mapper.lockTicket(item.ticketId()).orElseThrow(ProposalException::notFound); if (ticket.customerId() != user.id()) throw ProposalException.forbidden(); LocalDateTime now = LocalDateTime.now(ZoneOffset.UTC); if (mapper.shipReturn(returnId, request.carrierCode(), request.trackingNo(), now) != 1) throw ProposalException.invalid("RETURN_INVALID_TRANSITION", "当前退货单不允许填写物流"); if (mapper.transitionTicket(ticket.id(), ticket.status(), "RETURN_IN_TRANSIT", ticket.version(), now) != 1) throw ProposalException.ticketVersion(); mapper.insertTimeline(ids.nextId(), ticket.id(), "RETURN_SHIPPED", "CUSTOMER", user.id(), "用户填写退货物流", null, now); return mapper.findReturn(returnId).orElseThrow(ProposalException::notFound); }
    @Transactional public Object receiveReturn(AuthenticatedUser user, long returnId, ProposalRequests.Receive request) { requireAgent(user); var item = mapper.findReturn(returnId).orElseThrow(ProposalException::notFound); var ticket = mapper.lockTicket(item.ticketId()).orElseThrow(ProposalException::notFound); checkAgentAccess(user, ticket); LocalDateTime now = LocalDateTime.now(ZoneOffset.UTC); if (mapper.receiveReturn(returnId, now) != 1) throw ProposalException.invalid("RETURN_INVALID_TRANSITION", "当前退货单不允许确认收货"); if (mapper.transitionTicket(ticket.id(), ticket.status(), "RETURN_RECEIVED", ticket.version(), now) != 1) throw ProposalException.ticketVersion(); mapper.insertTimeline(ids.nextId(), ticket.id(), "RETURN_RECEIVED", user.role().name(), user.id(), "客服确认收货", null, now); return mapper.findReturn(returnId).orElseThrow(ProposalException::notFound); }
    @Transactional public Object inspectReturn(AuthenticatedUser user, long returnId, ProposalRequests.Inspect request) { requireAgent(user); var item = mapper.findReturn(returnId).orElseThrow(ProposalException::notFound); var ticket = mapper.lockTicket(item.ticketId()).orElseThrow(ProposalException::notFound); checkAgentAccess(user, ticket); LocalDateTime now = LocalDateTime.now(ZoneOffset.UTC); if (mapper.inspectReturn(returnId, request.result(), request.note(), now) != 1) throw ProposalException.invalid("RETURN_INVALID_TRANSITION", "当前退货单不允许检查"); String result = request.result().toUpperCase(Locale.ROOT); if ("PASSED".equals(result)) { var proposal = mapper.findProposal(item.proposalId()).orElseThrow(ProposalException::notFound); if (proposal.type().equals("RETURN_REFUND")) { if (mapper.refundItem(ticket.orderItemId(), Math.max(0, ticket.paidAmountCent() - ticket.refundedAmountCent()), now) != 1) throw ProposalException.invalid("PROPOSAL_AMOUNT_INVALID", "退款金额校验失败"); mapper.transitionTicket(ticket.id(), ticket.status(), "PROCESSING", ticket.version(), now); mapper.transitionTicket(ticket.id(), "PROCESSING", "RESOLVED", ticket.version()+1, now); mapper.deleteActiveGuard(ticket.id()); } else if (proposal.type().equals("EXCHANGE")) { mapper.startExchange(proposal.id(), now); mapper.transitionTicket(ticket.id(), ticket.status(), "PROCESSING", ticket.version(), now); long shipmentId = ids.nextId(); String shipmentNo = "EX" + String.format("%010d", Math.floorMod(shipmentId, 10_000_000_000L)); mapper.insertMockShipment(shipmentId, shipmentNo, ticket.orderId(), "REPLACEMENT", "MOCK-" + shipmentNo, now); mapper.markExchangeShipped(proposal.id(), shipmentId, now); } else if (proposal.type().equals("REPAIR")) { mapper.startRepairInspect(proposal.id(), now); mapper.transitionTicket(ticket.id(), ticket.status(), "PROCESSING", ticket.version(), now); long shipmentId = ids.nextId(); String shipmentNo = "RP" + String.format("%010d", Math.floorMod(shipmentId, 10_000_000_000L)); mapper.insertMockShipment(shipmentId, shipmentNo, ticket.orderId(), "REPAIR_RETURN", "MOCK-" + shipmentNo, now); mapper.markRepairShipped(proposal.id(), shipmentId, now); } } mapper.insertTimeline(ids.nextId(), ticket.id(), "RETURN_INSPECTED", user.role().name(), user.id(), "客服完成退货检查", request.result(), now); return mapper.findReturn(returnId).orElseThrow(ProposalException::notFound); }
